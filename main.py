...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. أوامر التنقل العام (تعمل دائمًا)
    ...
    # 3. معالجة الإدخال أثناء حالات الأدمن المتغيرة (admin_state)
    if user_id == ADMIN_ID and admin_state:
        if admin_state == "waiting_add_btn_name":
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO main_buttons (parent_id, text, type, content) VALUES (?, ?, 'menu', NULL)",
                (current_p, btn_name)
            )
            conn.commit()
            conn.close()
            context.user_data.pop('admin_state', None)
            await update.message.reply_text(f"تمت إضافة الزر ({btn_name}) بنجاح! ✅")
            await show_menu(update, context, parent_id=current_p)
            return
            ...
            # اقتطاع أول 20 حرف لتسمية الزر
            btn_title = msg_content[:20] + "..." if len(msg_content) > 20 else msg_content
            
            conn = sqlite3.connect("tree_bot.db")
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO main_buttons (parent_id, text, type, content) VALUES (?, ?, 'content', ?)",
                (current_p, btn_title, msg_content)
            )
            conn.commit()
            conn.close()
            context.user_data.pop('admin_state', None)
            await update.message.reply_text("تمت إضافة الرسالة بنجاح! ✅")
            await show_menu(update, context, parent_id=current_p)
            return
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE main_buttons SET content = ? WHERE parent_id = ? AND type = 'content'",
                (text, current_p)
            )
            conn.commit()
            conn.close()
            context.user_data.pop('admin_state', None)
            await update.message.reply_text("تم تحديث المحتوى بنجاح! ✅")
            await show_menu(update, context, parent_id=current_p)
            return

    # 4. اختيار الأزرار من وضع المحرر (بدء تتبع الحالة)
    if user_id == ADMIN_ID and admin_mode:
        if text == "➕ إضافة زر":
            ...
            await update.message.reply_text("أرسل محتوى الرسالة النصية التي ستظهر عند الضغط على هذا الزر:")
            return

    # 5. أمر تعديل المحتوى (خارج وضع المحرر أو داخله)
    if user_id == ADMIN_ID and text == "✏️ تعديل المشاركات (المحتوى)":
        context.user_data['admin_state'] = "waiting_edit_content"
        await update.message.reply_text("أرسل المحتوى الجديد ليتم حفظه في هذا القسم:")
        return

    # 6. التنقل واستعراض الأزرار
    current_p = context.user_data.get('current_parent_id', 0)
    conn = sqlite3.connect("tree_bot.db")
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, type, content, text FROM main_buttons WHERE parent_id = ? AND text = ?",
        (current_p, text)
    )
    btn = cursor.fetchone()
    conn.close()
